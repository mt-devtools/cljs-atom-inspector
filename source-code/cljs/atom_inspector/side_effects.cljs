...
(defn inspect-path!
  ; @important
  ; Only map items are browsable with the inspector.
  ;
  ; @description
  ...
  ; @param (vector) path
  [inspector-id path]
  (common-state/assoc-state! :atom-inspector :inspectors inspector-id :meta-items {:inspected-path path}))
...
(defn go-home!
  ...
  ; @param (keyword) inspector-id
  [inspector-id]
  (common-state/assoc-state! :atom-inspector :inspectors inspector-id :meta-items {:inspected-path []}))
...
(defn go-up!
  ...
  [inspector-id]
  (let [inspected-path (env/get-inspected-path  inspector-id)
        parent-path    (vector/remove-last-item inspected-path)]
       (common-state/assoc-state! :atom-inspector :inspectors inspector-id :meta-items {:inspected-path parent-path})))
...
(defn remove-inspected-item!
  ...
  (let [atom-ref       (env/get-atom-ref       inspector-id)
        inspected-path (env/get-inspected-path inspector-id)
        inspected-item (env/get-inspected-item inspector-id)]
       (common-state/assoc-state! :atom-inspector :inspectors inspector-id :meta-items :bin inspected-item)
       (if (env/root-level? inspector-id)
           (reset! atom-ref nil)
...
(defn restore-inspected-item!
  ...
  (let [atom-ref       (env/get-atom-ref       inspector-id)
        inspected-path (env/get-inspected-path inspector-id)
        removed-item   (env/get-bin            inspector-id)]
       (common-state/dissoc-state! :atom-inspector :inspectors inspector-id :meta-items :bin)
       (if (env/root-level? inspector-id)
...
(defn toggle-raw-view!
  ...
  ; @param (keyword) inspector-id
  [inspector-id]
  (common-state/update-state! :atom-inspector :inspectors update-in [inspector-id :meta-items :raw-view?] not))
  ...
  (let [atom-ref       (env/get-atom-ref       inspector-id)
       ...
       (if-let [edit-mode? (env/edit-mode? inspector-id)]
               ...
               (let [inspected-item (env/get-inspected-item inspector-id)
                    ...
                    (common-state/assoc-state! :atom-inspector :inspectors inspector-id :meta-items :edit-copy unparsed-item))))

  ; Toggling the edit mode switch
  (common-state/update-state! :atom-inspector :inspectors update-in [inspector-id :meta-items :edit-mode?] not))
